        ...
        <Container maxW="6xl">
          ...
          <SimpleGrid columns={{ base: 1, md: 2, lg: 2 }} spacing={{ base: 8, md: 10 }}>
              {projects.map((project, idx) => (
                <motion.div key={project.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: idx * 0.06 }}>
                  <Box p={{ base: 3, md: 4 }}>
                    <Box>
                      <Box
                        display="block"
                      ...
                      >
                        <Box h={{ base: '260px', md: '320px' }} bg="gray.200" position="relative" overflow="hidden">
                          <Image src={project.image ? project.image : makePlaceholder(project.title)} alt={project.title} objectFit="cover" w="full" h="full" transition="transform 0.3s" _hover={{ transform: 'scale(1.05)' }} />
                        </Box>

                        <Box p={{ base: 6, md: 8 }}>
                          <Badge px={3} py={1} bg="blue.50" color="blue.600" borderRadius="full" fontSize="xs" mb={3}>Project</Badge>

                          <Heading as="h3" fontSize="18px" mb={2} fontWeight="medium">{project.title}</Heading>

                          <Text fontSize="14px" color="gray.600" mb={4}>{project.description}</Text>

                          <HStack spacing={2} flexWrap="wrap">
                            {project.techStack.slice(0, 3).map((tech, i) => (
                              <Badge key={i} px={2} py={1} bg="gray.100" color="gray.700" borderRadius="md" fontSize="xs">{tech}</Badge>
                            ))}
                          </HStack>
